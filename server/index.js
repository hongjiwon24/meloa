...
app.delete('/api/tracks/:id', async (req, res) => {
  try {
    ...
    const audioPath = path.join(__dirname, 'uploads', 'audio', track.filename);
    const imagePath = path.join(__dirname, 'uploads', 'images', track.coverImage);

    // 오디오 파일 삭제
    try {
      if (fs.existsSync(audioPath)) {
        await fs.promises.unlink(audioPath);
        console.log('✅ 오디오 파일 삭제됨:', track.filename);
      }
    } catch (err) {
      console.warn('❌ 오디오 삭제 실패:', err.message);
    }
    ...
    try {
      if (fs.existsSync(imagePath)) {
        ...
        console.log('✅ 이미지 파일 삭제됨:', track.coverImage);
      }
    } catch (err) {
